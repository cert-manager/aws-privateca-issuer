...
install_aws_load_balancer() {
    ...
    helm install aws-load-balancer-controller eks/aws-load-balancer-controller -n kube-system --set clusterName=$CLUSTER_NAME --set image.repository=public.ecr.aws/eks/aws-alb-ingress-controller --set env.AWS_ACCESS_KEY_ID="$AWS_ACCESS_KEY_ID" --set env.AWS_SECRET_ACCESS_KEY="$AWS_SECRET_ACCESS_KEY" --set env.AWS_SESSION_TOKEN="$AWS_SESSION_TOKEN" --set env.AWS_REGION="$AWS_REGION" >/dev/null 2>&1
