...
func getCaArn(caType string) string {
	switch caType {
		case "RSA":   return testContext.rsaCaArn
		case "ECDSA": return testContext.ecCaArn
		case "XA":    return testContext.xaCaArn
		default:      panic(fmt.Sprintf("Unknown CA Type: %s", caType))
	}
}
...
func getCertSpec(certType string) cmv1.CertificateSpec {
	switch certType {
		case "RSA":            return getBaseCertSpec(certType)
		case "ECDSA":          return getBaseCertSpec(certType)

		// For simplicity, we use RSA as the base for these. This can be further generalized if desired.
		case "SHORT_VALIDITY": return getCertSpecWithValidity("RSA", 20, 5)
		case "CA":             return getCaCertSpec("RSA")
		default:               panic(fmt.Sprintf("Unknown Certificate Type: %s", certType))
	}
}
...
func (issCtx *IssuerContext) verifyCertificateState(ctx context.Context, condition string, status string) error {
	err := waitForCertificateState(ctx, testContext.cmClient, issCtx.certName, issCtx.namespace, condition, status)

	if err != nil {
		assert.FailNow(godog.T(ctx), "Certificate did not reach specified state, Condition = "+condition+", Status = "+status+": "+err.Error())
	}

	return nil
}

func (issCtx *IssuerContext) verifyCertificateRequestState(ctx context.Context, condition string, status string) error {
	certificateRequestName := fmt.Sprintf("%s-%d", issCtx.certName, 1)
	err := waitForCertificateRequestState(ctx, testContext.cmClient, certificateRequestName, issCtx.namespace, condition, status)

	if err != nil {
		assert.FailNow(godog.T(ctx), "Certificate Request did not reach specified state, Condition = "+condition+", Status = "+status+": "+err.Error())
	}
